...
class ParallaxLayer: public WorldLayer
{
protected:
...
public:
	...
	// METHODS DEFINED IN ParallaxLayer.cpp
	ParallaxLayer(){}
	ParallaxLayer(int initColumns, int initRows, int initTileWidth, int initTileHeight, int initZ, bool initCollidableTiles, Viewport *viewport, int worldWidth, int worldHeight);
	~ParallaxLayer();
	void	addRenderItemsToRenderList(RenderList *renderList, Viewport *viewport);
	void	addTile(pTile *initTile);
	...
	pTile*	getTile(int row, int column);
	...
	void	setTile(pTile *initTile, int row, int column);
};