...
{
	guiRenderListDraw->resetIterator();
	...
	while (guiRenderListDraw->hasNext())
	{
		if (rect != NULL)
			delete rect;
		rect = NULL;
		//itemToRender = guiRenderList->next();
		itemToRender = guiRenderListDraw->next();
...
void DirectXGraphics::renderWorldRenderList()
{
	...
	worldRenderListDraw->resetIterator();
	RenderItem itemToRender;
	LPDIRECT3DTEXTURE9 texture;
	RECT *rect = NULL;
	...
	while (worldRenderListDraw->hasNext())
	{
		if (rect != NULL)
			delete rect;
		rect = NULL;
		//itemToRender = worldRenderList->next();
		itemToRender = worldRenderListDraw->next();
