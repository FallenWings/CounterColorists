/*
	...
	See ParallaxLayer.h for a class description.
*/
...
/*
	ParallaxLayer - Constructor, it initializes the data structures
	...
	addTile method.
*/
ParallaxLayer::ParallaxLayer(	int initColumns, 
						int initRows,
						int initTileWidth, 
						int initTileHeight,
						int initZ,
...
/*
	...
	frame for each ParallaxLayer in a game world while the game is
...
*/
void ParallaxLayer::addRenderItemsToRenderList(RenderList *renderList,
											Viewport *viewport)
{
	...
	if (getRows() == 1  && getColumns() == 1)
		ratioX = (double) (layerWidth - viewport->getViewportWidth()) / (worldWidth);
	else
		ratioX = (double) (layerHeight) / (worldWidth);
	if (getRows() == 1  && getColumns() == 1)
		ratioY = (double) (layerHeight - viewport->getViewportHeight()) / (worldHeight);
	else
		ratioY = (double) (layerWidth) / (worldHeight);

	float viewportX = (float)((viewport->getViewportX())*ratioX);
	float viewportY = (float)((viewport->getViewportY())*ratioY);
	...
	for (int i = startRow; i <= endRow; i++)
	{
		for (int j = startCol; j <= endCol; j++)
		{
			...
			{
				tileY = (i * tileHeight) - viewportY; // todo static size
			}
...
*/
void ParallaxLayer::addTile(pTile *initTile)
{
...
*/
int ParallaxLayer::calculateAndSetLayerWidth()
{
...
*/
int ParallaxLayer::calculateAndSetLayerHeight()
{ 
...
*/
int ParallaxLayer::calculateViewportEndColumn(int viewportX,
										   int viewportWidth)
{
...
*/
int ParallaxLayer::calculateViewportEndRow(int viewportY,
										int viewportHeight)
{
...
/*
	...
	need to render tiles in columns that are not in the viewport.
*/
int ParallaxLayer::calculateViewportStartColumn(int viewportX)
{
...
/*
	...
	need to render tiles in rows that are not in the viewport.
*/
int ParallaxLayer::calculateViewportStartRow(int viewportY)
{
...
/*
	...
	from 2D to 1D.
*/
pTile* ParallaxLayer::getTile(int row, int column) 
{
...
*/
void ParallaxLayer::setTile(pTile *initTile, int row, int column)
{
	int cellIndex = (row * columns) + column;
