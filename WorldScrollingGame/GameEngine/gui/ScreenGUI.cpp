...
ScreenGUI::ScreenGUI()
{
	...
	healthBarImages = new vector<OverlayImage*>();
	screenName = NULL;
}
...
void ScreenGUI::addHealthBarImage(OverlayImage *imageToAdd)
{
	healthBarImages->push_back(imageToAdd);
}
...
*/
void ScreenGUI::addRenderItemsToRenderList(RenderList *renderList, Game *game)
{
	...
	{
		int currentHealth = AnimatedSprite::getPlayerHealth();
		for(int i = 0; i < currentHealth; i++)
		{
			OverlayImage *image = healthBarImages->at(i);
